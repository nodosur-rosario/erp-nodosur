...
export async function authorizeInvoice(
  ...
  try {
    ...
    if (environment === "simulation") {
      ...
      try {
        // WSAA Authentication Caching & Ticket Lookup para el servicio 'wsfe'
      ...
      } catch (realNetworkErr: any) {
        console.error("⚠️ Real WSAA/WSFE call failed. Falling back dynamically to Local Simulation Mode:", realNetworkErr);
        ...
        if (!response.ok) {
          const errJson = await response.json();
          throw new Error(`Simulation fallback failed: ${errJson.error || "Falla en el simulador local."}`);
        }
        ...
        return {
          ...
          cbte_nro: result.cbte_nro,
          qr_url: result.qr_url,
          error: `⚠️ Modo Simulación activado debido a falla real: ${realNetworkErr.message || realNetworkErr}`
        };
      }
    }
  } catch (err: any) {
